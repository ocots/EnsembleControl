...
init_guess = (
    state = t -> begin
        ...
        s1_interp = config.initial_position[1] + t/config.time_final * (0.0 - config.initial_position[1])
        s2_interp = config.initial_position[2] + t/config.time_final * (0.0 - config.initial_position[2])
        # Heights remain at initial values
        return [s1_interp, s2_interp, h0...]
    end,
    control = t -> config.reference_velocity,
    variable = config.time_final
)

sol = solve(ocp, init=init_guess;
    grid_size = 500,  # More discretization points
    tol = 1e-8,       # Tolerance
